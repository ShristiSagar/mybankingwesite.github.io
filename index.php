<html>
<head>
    <title>Sagar Bank</title>
    <link rel="shortcut icon" href="images/5.png">
   
	 <link rel="stylesheet" href="index_button.css">
	<style>
	button{
		background-color:#884EA0;
	}
	body
	{
		text-align:center;
	}
		body{
		background-image:url("images/31.jpg");
		background-position: center;
		background-repeat: no-repeat;
		background-size: cover;
	}
	</style>
</head>
<body >

	<div id="header">
       <br>
       <h1 style=" font-family:Agency FB; font-size: 70px; color:#FF337A;text-shadow: 2px 2px black;"> Sparks Bank </h1>
       <h2 style=" font-family:Agency FB; font-size: 55px;color:#F3FF33 ;text-shadow: 2px 2px black;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;..We Understand Ur World.. </h2>
    </div>
        <div id="section">
            <table>
                <tr></tr>
                <tr><br>
				<a href="getdetail.php">
               <button class="primary_btn" id="button "type="button" href="getdetail.php">View users</button>
                </a>

               
                </tr>

                <tr>        
               <br> <br> <br>
			   <a href="transfer.php">
			   <button class="primary_btn" id="button" type="button">Credit Transfer</button>
               </a>
               
               </tr>
			   
			    <tr>        
               <br> <br> <br>
			   <a href="transaction.php">
			   <button class="primary_btn" id="button" type="button">All Transaction</button>
               </a>

            </table>
    </div>
	           
</body>
</html>